before_config do
  ...
  if prefer :capistrano, "vserver"
    ...
    prefs[:deploy_repo]   = `git remote -v | perl -nE 'm/^\w+\s+(\S+)\s+\(fetch\)/ && print $!'` unless prefs.has_key? :deploy_repo
    prefs[:deploy_repo]   = ask_wizard "Repository to fetch from:" unless prefs.has_key? :deploy_repo
...
after_bundler do
  ...
  run "capify ."
  gsub_file 'Capfile', /\s*# load 'deploy\/assets'/, "load 'deploy/assets'"

  remove_file 'config/deploy.rb'
  create_file 'config/deploy.rb' do <<-DEPLOY
    require "bundler/capistrano"

    set :bundle_flags, "--deployment --binstubs"

    set :application, "#{app_name}"
    set :repository,  "#{prefs[:deploy_repo]}"
    # set :branch, 'master'
...
DEPLOY
  ...
  if prefer :mysql or prefer :database, 'mysql'
    copy_from_repo 'config/deploy/database.yml.erb', :repo => repo
    inject_into_file 'config/deploy.rb', 'require "./config/deploy/capistrano_database"', :after => 'require "bundler/capistrano"'
  end
  ...
  gsub_file 'config/environment/production.rb', /config.action_mailer.delivery_method = :smtp/, "config.action_mailer.delivery_method = :sendmail"
end
